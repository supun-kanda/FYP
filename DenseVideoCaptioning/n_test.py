...
def test(options):
    ...
    #features = h5py.File('/home/jayathu/DAMBA/sub_activitynet_v1-3_stride_64frame.c3d.hdf5', 'r')
    stride = 4
    c3d_resolution = 16
    #frame_rates = json.load(open('dataset/ActivityNet_Captions/video_fps.json'))
    frame_rates = np.load('/home/jayathu/DAMBA/fps.npy') 
   ...
   ###########################################for vid in test_ids############ :#################################################################################### 
    ...
    video_feat_fw =  np.load('/home/supunK/GIT/feats.npy')
    #video_feat_fw = np.zeros((1,500))*0.5 
    #print(type(video_feat_fw))
    ...
    fps = frame_rates[vid]
    #fps = np.load('/home/supunK/GIT/fps.npy')
   ...
   ##################################################end of for loop############################################################################################### 
	    ...
	    #with open('sentences.txt', 'a') as the_file:
            # 	the_file.write('%s\n'%(out_sentence.encode('utf-8')))
	
	if(np.any(maxx<sentence_confidences)):
		indx = np.argmax(sentence_confidences)
		proposal = proposal_infos[indx]
		result = {'timestamp': proposal['timestamp'], 'proposal_score': float(proposal['score']), 'sentence': out_sentences[indx], 'sentence_confidence': float(sentence_confidences[indx])} 
	...
	#print(sentence_confidences)
		print('sentences confidence %f : %s '%(sentence_confidences[indx],out_sentences[indx]))
		maxx = sentence_confidences[indx]
	#print(out_sentences[i] for i in range(len(out_sentences)))
	#for i in range(len(out_sentences)):
	#	print(out_sentences[i])
